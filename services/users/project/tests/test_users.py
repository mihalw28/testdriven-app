...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn("michal@waszak.com was added!", data["message"])
            self.assertIn("success", data["status"])
    ...
    def test_add_user_invalid_json_keys(self):
        """
        Ensure error is thrown if the JSON obect does not have a username key.
        """
        with self.client:
            response = self.client.post(
                "/users",
                data=json.dumps(
                    {"email": "michal@waszak.com", "password": "randomrandom"}
                ),
                content_type="application/json",
            )
            data = json.loads(response.data.decode())
    ...
    def test_single_user(self):
        ...
        user = add_user("michal", "michal@waszak.com", "randomrandom")
        with self.client:
            response = self.client.get(f"/users/{user.id}")
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn("michal", data["data"]["username"])
            self.assertIn("michal@waszak.com", data["data"]["email"])
            self.assertIn("success", data["status"])

    def test_all_users(self):
        """Ensure get all users behaves correctly."""
        add_user("michal", "michal@waszak.com", "randomrandom")
        add_user("elon", "elon@musk.com", "randomrandom")
        with self.client:
    ...
    def test_single_user_no_id(self):
        ...
        with self.client:
            response = self.client.get("/users/cos")
            data = json.loads(response.data.decode())
    ...
    def test_main_with_users(self):
        ...
        """
        add_user("michal", "michal@waszak.com", "randomrandom")
        add_user("jan", "jan@kowalski.com", "randomrandom")
        with self.client.get("/"):
            response = self.client.get("/")
            self.assertEqual(response.status_code, 200)
            self.assertIn(b"All Users", response.data)
            self.assertNotIn(b"<p>No users!<p>", response.data)
            self.assertIn(b"michal", response.data)
    ...
    def test_main_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn(b"All Users", response.data)
            self.assertNotIn(b"<p>No users!<p>", response.data)
            self.assertIn(b"donald", response.data)
