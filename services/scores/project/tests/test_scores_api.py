...
class TestScoresService(BaseTestCase):
    ...
    def test_all_scores_by_user_id(self):
        ...
        with self.client:
            response = self.client.get(
                f"/scores/user", headers=({"Authorization": "Bearer test"})
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
    ...
    def test_single_score_by_user_id_no_id(self):
        ...
        with self.client:
            response = self.client.get(
                "/scores/user/blah", headers=({"Authorization": "Bearer test"})
            )
            data = json.loads(response.data.decode())
    ...
    def test_single_score_incorrect_id(self):
        ...
        with self.client:
            response = self.client.get(
                "/scores/user/999", headers=({"Authorization": "Bearer test"})
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 404)
            self.assertIn("Score does not exist", data["message"])
    ...
    def test_add_score(self):
        ...
        with self.client:
            response = self.client.post(
                "/scores",
                data=json.dumps({"exercise_id": 1, "correct": False}),
